...
func main() {
	...
	switch ctx.Command() {
	case "add <name> <host>":
		code = utils.AddHost(CLI.Add.Name, CLI.Add.Host, CLI.Add.Port)
		break
	case "update <name> <host>":
		host := utils.Host{CLI.Update.Name, CLI.Update.Host, CLI.Update.Port}
		code = utils.UpdateHost(CLI.Update.Name, host)
	case "setup <name>":
